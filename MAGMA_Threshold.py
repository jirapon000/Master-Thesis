...
#
#  PER TURN
#  Q agent   → MIRT evidence scores → pick best unasked domain → ask
#              Skip domain if accumulated evidence > CONFIRM_THRESHOLD
...
# =============================================================================
#  ADAPTIVE: MIRT Evidence Threshold
#  - Fixed question order, skip domain if accumulated evidence > CONFIRM_THRESHOLD
#  - PMI used for correlation propagation only, not for question selection
...
MAX_FOLLOWUPS       = 2
CORR_THRESHOLD      = 0.5
CORR_PROPAGATION    = 0.1 #Control how much evidence spreads to correlated domain after each answer
...
# =============================================================================
#  PHQ-8 ITEMS  (unified — your ITEMS list + MAGMA's PHQ8_HYPOTHESES merged)
# =============================================================================
ITEMS = [
...
# =============================================================================
#  STEP 7 — PARTICIPANT PROFILE CLASSIFIER (MAGMA)
# =============================================================================
...
# =============================================================================
#  STEP 8 — SIMULATE CLIENT ANSWER (MAGMA — kept intact)
# =============================================================================
...
# =============================================================================
#  STEP 9 — YOUR QUESTION GENERATION (replaces MAGMA question_template)
# =============================================================================
...
# =============================================================================
#  STEP 10 — YOUR TRANSCRIPT-BASED SCORING (replaces MAGMA scoring_template)
# =============================================================================
...
# =============================================================================
#  STEP 11 — AGENTSTATE (MAGMA + your new fields)
# =============================================================================
...
# =============================================================================
#  STEP 13 — 6 AGENT PROMPTS / TEMPLATES
...
#  │  Agent 2 — Participant Agent   (MAGMA rapport-tiered simulator)         │
#  │  Agent 3 — Clarification Agent (MAGMA NLI quality check)               │
#  │  Agent 4 — Alignment Agent     (MAGMA NLI + YOUR .corr() statistics)   │
#  │  Agent 5 — Navigation Agent    (MAGMA escalation + PMI stop logic)     │
#  │  Agent 6 — Scoring Agent       (YOUR transcript-based PHQ-8 scorer)    │
#  └─────────────────────────────────────────────────────────────────────────┘
# =============================================================================

# -----------------------------------------------------------------------------
#  AGENT 1 — QUESTION AGENT TEMPLATE
#  Source: YOUR generate_dynamic_question (Table A.5 approach)
#  PMI shortlists top-3 candidate domains by information gain.
#  This template is passed to the OpenAI client directly (not LangChain chain)
#  because it requires dynamic f-string injection of evidence + candidates.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#  AGENT 2 — PARTICIPANT AGENT TEMPLATE
#  Source: MAGMA simulate_client_answer — kept intact
#  Rapport-tiered unmasking: level1 (open) / level2 (guarded) / level3 (resistant)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#  AGENT 3 — CLARIFICATION AGENT TEMPLATE
#  Source: MAGMA clarification_template — kept intact
#  Checks: Timeframe / Vagueness / Relevance (Zero-Shot CoT)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#  AGENT 4 — ALIGNMENT AGENT TEMPLATE
#  Source: MAGMA alignment_template (NLI semantic layer) +
#          YOUR check_corr_alignment (.corr() statistical layer)
#  The template handles the NLI/semantic half.
#  The .corr() check runs in Python code inside alignment_node and its output
#  is merged with this template's verdict before passing to navigation_node.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#  AGENT 5 — NAVIGATION AGENT TEMPLATE
#  Source: MAGMA navigation_template — kept intact
#  Now also receives corr_flags (from YOUR .corr() alignment) so the agent
#  can factor in statistical contradictions alongside NLI ones.
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#  AGENT 6 — SCORING AGENT TEMPLATE
#  Hybrid: MAGMA's full PCoT scoring rules + YOUR single-call transcript approach.
#  Scores all 8 PHQ-8 domains in ONE API call (not 8 separate calls).
#  MAGMA's per-item loop is replaced by reading the full transcript at once —
#  cross-item context (equivalent to alignment_context) is naturally present
#  in the transcript itself, so no separate injection is needed.
#  Output includes: score, confidence, data_sufficiency, reasoning_chain (5 steps).
# -----------------------------------------------------------------------------
...
def question_node(state: AgentState):
    ...
    threshold_gain_log = list(state.get("threshold_gain_log", []))
    ...
    # PHQ-8 ITEMS: use YOUR PMI-based dynamic question
        ...
        if state.get("followup_count", 0) > 0 and nav_instr:
            ...
            # Fixed-order selection — ask in PHQ8_HYPOTHESES order, skip if evidence > CONFIRM_THRESHOLD
            ...
            if not remaining_keys:
                ...
                # Pick next domain in fixed order (I1→I2→...→I8), skip if already confirmed
                best_domain = None
                skipped     = []
                for h in PHQ8_HYPOTHESES:
                    if h["phq_key"] in asked_keys:
                        continue
                    evidence_score = accumulated.get(h["phq_key"], 0)
                    if evidence_score > CONFIRM_THRESHOLD:
                        skipped.append((h["phq_key"], round(evidence_score, 3)))
                        continue
    ...
    return {
        ...
        "threshold_gain_log":         threshold_gain_log,
    }


# ── NODE 2: Participant Node (MAGMA simulate_client_answer — kept intact)
def participant_node(state: AgentState):
    ...
    # Also mark what GPT actually selected as asked
    ...
    # ── Threshold Gain Log (logged after accumulated_evidence is updated)
    threshold_gain_log = list(state.get("threshold_gain_log", []))
    remaining_log      = [h for h in PHQ8_HYPOTHESES if h["phq_key"] not in asked_keys]
    skipped_count      = sum(
        1 for h in PHQ8_HYPOTHESES
        if h["phq_key"] not in asked_keys
        and accumulated.get(h["phq_key"], 0) > CONFIRM_THRESHOLD
    )
    ...
    log_entry = {
        ...
        "evidence_score":    round(accumulated.get(current_phq_key, 0), 4),
        "skipped_count":     skipped_count,
        # Full evidence snapshot for all 8 domains
    ...
    threshold_gain_log.append(log_entry)
...
# ── NODE 3: Clarification Node (MAGMA — kept intact)
...
def transition_node(state: AgentState):
    ...
    # Find next domain in fixed order, skip if evidence > CONFIRM_THRESHOLD
    asked_keys  = list(state.get("asked_phq_keys", []))
    accumulated = state.get("accumulated_evidence", {k: 0.0 for k in ITEMS})
    ...
    # Pick next in fixed order, skip if already confirmed
    eligible = None
    for h in remaining:
        evidence_score = accumulated.get(h["phq_key"], 0)
        if evidence_score > CONFIRM_THRESHOLD:
            print(f"   [Threshold Skip] ⏭️  {h['item_id']} ({h['label']}) — "
                  f"evidence {round(evidence_score, 3)} > {CONFIRM_THRESHOLD}, skipping.")
            continue
        eligible = h
        break
...
def batch_scoring_node(state: AgentState):
    ...
    # Build from state["transcript"] -- always populated, never dropped by LangGraph
    conv_hist_dicts = [
...
# =============================================================================
#  STEP 15 — GRAPH ASSEMBLY (MAGMA LangGraph structure — kept intact)
# =============================================================================
def build_graph():
...
# =============================================================================
#  STEP 17 — MAIN (MAGMA output structure — kept exactly)
# =============================================================================
def main():
    ...
    dirs = {
        ...
        "gn": os.path.join(base_dir, "Threshold_Gains")
    }
    ...
    # H. Threshold Gain Log CSV
    gain_path = os.path.join(dirs["gn"], f"Threshold_Gains_{args.pid}.csv")
    gain_records = final_state.get("threshold_gain_log", [])
    for r in gain_records:
        r["PID"] = args.pid
    if gain_records:
        keys = ["PID", "turn_index", "item_id", "domain_asked", "evidence_score",
                "skipped_count",
